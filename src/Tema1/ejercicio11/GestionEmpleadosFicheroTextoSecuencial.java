...
public class GestionEmpleadosFicheroTextoSecuencial {
	...
	static private class EmployeeSM{
		...
		final static boolean isValidString(String s){
			if(s.contains(EOFIELD) || s.contains(EORECORD)) return false;
			else return true;
		}	
	}
	...
	private static class Menu{
		...
		enum opcionesMenu {create, open, add, erase, change, show, close, end}
		...
		while ((opcion=Menu.menu())!=Menu.opcionesMenu.end){
				...
				switch (opcion){		                       
					...
					case create:{  String route;
                                  ...
                                  if (employeeFile.isFile()) employeeFile.delete();
                                  ...
                                  }catch(IOException ioe){System.err.println("I/O error: Is not posible to create the file");}
                                  
					            }
						        break;
					           
					case open: { String route;
                                ...
                                if (!employeeFile.isFile()) System.err.println("There isn't exists the file " + route);
	             ...
	             //�*! We should ensure the unicity of each employee's code, for example: another file with not-used codes
	              ...
	              //�*! We should ensure the field format correctness ('nnnnlll' or 'llnnnnll')
	                           ...
	                           if(employeeRAMrecord.carId.equals("no car")) employeeRAMrecord.carId=""; //If the employee has no car, this file should be empty
	                          ...
	                          */
	                           ...
	                           try{
	                            ...
	                            //3c-We close the 'FileWriter' object, and therefore, we 'close' the file
	                                employeeFileWriter.close();
	                           }catch(IOException ioe){System.err.println("An I/O error occurs and no employee was added");}                
			...
			/*------>*/    boolean b2=employeeFileTmp.renameTo(employeeFile);  
					              ...
					              if (!found)  System.err.printf("Employee record with code %s not found\n",code);

				               } catch (IOException ioe) {System.err.println("An I/O error occurs and no employee was deleted");}
			}
				               break;
					...
					case show:{ System.out.println("We search starting from the first record of the sequential access file");
